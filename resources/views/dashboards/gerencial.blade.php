        ...
        <div class="p-5">
          ...
          <div class="overflow-x-auto">
            <table class="w-full text-xs">
              <thead>
                <tr class="border-b border-slate-100 bg-slate-50/70">
                  <th class="text-left py-2.5 pr-3 pl-3 font-semibold text-slate-500">Mes de compromiso</th>
                  <th class="text-center py-2.5 px-2 font-semibold text-slate-500">Pedidos</th>
                  <th class="text-right py-2.5 px-2 font-semibold text-slate-500">Cant. pendiente</th>
                  <th class="text-right py-2.5 pl-2 pr-3 font-semibold text-slate-500">Total comprometido</th>
                </tr>
              </thead>
              <tbody class="divide-y divide-slate-50">
                <template x-for="(m, idx) in canastaResumen" :key="idx">
                  <tr class="hover:bg-amber-50/60 cursor-pointer transition-colors" @click="toggleMes(m)">
                    <td class="py-2.5 pr-3 pl-3 font-medium text-slate-800">
                      <span class="inline-block mr-1 text-slate-400" x-text="mesAbierto===claveMes(m) ? '▾' : '▸'"></span>
                      <span x-text="nombresMesCorto[m.mes_entrega-1] + ' ' + m.anio_entrega"></span>
                      <span x-show="filtro.cia===0" class="text-slate-400 font-normal" x-text="' · '+ciaName(m.compania)"></span>
                    </td>
                    <td class="text-center py-2.5 px-2 text-slate-500" x-text="m.num_pedidos"></td>
                    <td class="text-right py-2.5 px-2 text-slate-600 tnum" x-text="Number(m.cant_pendiente||0).toLocaleString('es-CO')"></td>
                    <td class="text-right py-2.5 pl-2 pr-3 font-bold text-amber-700 whitespace-nowrap" x-text="money(m.total_comprometido)"></td>
                  </tr>
                </template>
                <tr x-show="canastaResumen.length === 0"><td colspan="4" class="py-6 text-center text-sm text-slate-400">Sin pedidos pendientes desde el mes actual en adelante.</td></tr>
              </tbody>
            </table>
          </div>

          {{-- Detalle del mes seleccionado --}}
          <div x-show="mesAbierto" class="mt-4 border-t border-slate-100 pt-4">
            <div class="overflow-x-auto max-h-72 overflow-y-auto">
              <table class="w-full text-xs">
                <thead>
                  <tr class="border-b border-slate-100 bg-slate-50/70 sticky top-0">
                    <th class="text-left py-2 px-3 font-semibold text-slate-400 uppercase tracking-wider">Cliente</th>
                    <th class="text-left py-2 px-3 font-semibold text-slate-400 uppercase tracking-wider">Documento</th>
                    <th class="text-left py-2 px-3 font-semibold text-slate-400 uppercase tracking-wider">Ítem</th>
                    <th class="text-right py-2 px-3 font-semibold text-slate-400 uppercase tracking-wider">Entrega</th>
                    <th class="text-right py-2 px-3 font-semibold text-slate-400 uppercase tracking-wider">Valor</th>
                    <th class="text-left py-2 px-3 font-semibold text-slate-400 uppercase tracking-wider">Estado</th>
                  </tr>
                </thead>
                <tbody class="divide-y divide-slate-50">
                  <template x-for="(l, idx) in canastaDetalleMesAbierto" :key="idx">
                    <tr class="hover:bg-slate-50/60 transition-colors">
                      <td class="py-2 px-3 font-medium text-slate-700" x-text="l.cliente"></td>
                      <td class="py-2 px-3 font-mono text-slate-500 whitespace-nowrap" x-text="l.nro_documento"></td>
                      <td class="py-2 px-3 text-slate-600" x-text="l.desc_item"></td>
                      <td class="py-2 px-3 text-right whitespace-nowrap text-slate-500" x-text="l.fecha_entrega ? new Date(l.fecha_entrega).toLocaleDateString('es-CO') : '—'"></td>
                      <td class="py-2 px-3 text-right font-semibold whitespace-nowrap text-slate-700" x-text="money(l.valor_subtotal)"></td>
                      <td class="py-2 px-3 text-slate-500" x-text="l.estado"></td>
                    </tr>
                  </template>
                  <tr x-show="canastaDetalleMesAbierto.length === 0"><td colspan="6" class="py-6 text-center text-sm text-slate-400">Sin líneas pendientes para este mes.</td></tr>
                </tbody>
              </table>
            </div>
          </div>
...
function informeComercial(datos){
  ...
  return {
    ...
    claveMes(m){ return m.compania+'-'+m.anio_entrega+'-'+m.mes_entrega; },
    toggleMes(m){ const k = this.claveMes(m); this.mesAbierto = (this.mesAbierto === k) ? null : k; },
    get canastaDetalleMesAbierto(){
      return this.mesAbierto ? this.canastaDetalle.filter(l => this.claveMes(l) === this.mesAbierto) : [];
    },
